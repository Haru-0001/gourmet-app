...
// 最初に表示するボタン
export const FirstButton= (props: {time:ThemeTimeType, pathName: string}) => {
    const time = props.time;
    const hoverTheme = themeColor[time].primaryHover
    const pathName = props.pathName;
    return(
        <Link href={pathName}>
            <button className={`flex justify-center items-center rounded-full size-80 border-4 border-slate-800/30 shadow-btn ${hoverTheme}`}>
